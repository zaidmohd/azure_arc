...
* The script will also deploy the sample app and NGINX ingress to Azure Arc-enabled Kubernetes cluster

* User is veryfing the cluster and make sure the extension is deployed.

* User is checking the secret value mounted in pod.

* User is checking the application ingress.
...
* Edit the environment variables in the script to match your environment parameters followed by running the ```. ./azure_keyvault_k8s_extension_ingress.sh``` command.
  ...
  * Create Resource Group, Key Vault and certificate
  ...
  * Deploy the Ingress and the application
...
```bash
kubectl exec busybox-secrets-sync -- ls /mnt/secrets-store/
```
...
```bash
kubectl exec busybox-secrets-sync -- sh -c 'echo $SECRET_USERNAME'
```
